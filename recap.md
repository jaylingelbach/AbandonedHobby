...
# Shipping data model and server calcs 11/04/25

## Walkthrough

- Expands per-item and product-level shipping, centralizes USD↔cents helpers, computes and persists shipping in checkout and webhook order creation, backfills/merges prior amounts (including stripe fees and shipping), and updates cart store, UI, and types to carry shipping snapshots and cents-based totals.

## New Features

- Products support per-item shipping modes (free, flat, calculated) and flat-fee settings.
- Product pages & cards show shipping badges; cart button saves per-product shipping snapshots.
- Checkout and sidebar now show subtotal, shipping, and total separately; shipping included in checkout flow.
- Orders and order views include detailed shipping totals and per-line shipping breakdowns.

## Style

- Minor UI sizing and layout refinements for product/gallery elements.

## File changes

### Stripe webhook & order backfill

- src/app/(app)/api/stripe/webhooks/route.ts
  - Switched to OrderItemOutput[]; expanded amounts to a richer AmountsShape (adds subtotalCents, taxTotalCents, shippingTotalCents, discountTotalCents, platformFeeCents, stripeFeeCents, sellerNetCents); preserve/merge existingAmounts when backfilling stripe fees; compute/integrate shipping amount from session/items.

### Orders collection & validation

- src/collections/Orders.ts
  - Added beforeValidate hook to normalize per-item shipping (shippingMode, quantity, shippingFeeCentsPerUnit, shippingSubtotalCents), new ShippingMode type and normalizeMode helper, and per-line shipping fields with validations.

### Products collection & product-level shipping

- src/collections/Products.ts, src/modules/products/server/procedures.ts
  - Added product shipping fields (shippingMode, shippingFlatFee), pre-change hook clearFlatFeeWhenNotFlat, and expose shippingFeeCentsPerUnit in product payload.

### Order & payload types

- src/modules/orders/types.ts, src/payload-types.ts, src/modules/stripe/build-order-items.ts
  - New exported ShippingMode, OrderItemCore, OrderAmounts, OrderForBuyer; OrderItemOutput extended with shippingMode, shippingFeeCentsPerUnit, shippingSubtotalCents; build-order-items computes per-line shipping fields.

### Money utilities

- src/lib/money.ts, src/lib/utils.ts
  - New src/lib/money.ts with guards and converters: isFiniteNumber, usdNumberToCents, usdStringToCents, usdToCents, centsToUsdNumber, sumCents; removed old usdToCents from src/lib/utils.ts.

### Server order totals & shipping derivation

- src/lib/server/orders/lock-and-calc-amounts.ts
  - Adds shipping derivation: prefer explicit shipping, else sum flat per-line fees, else call quoteCalculatedShipping() (stub) with safe fallbacks; derive item subtotals from available fields and include shipping in total computation and persisted amounts.

### Shipping quote stub

- src/modules/shipping/quote.ts
  - New module exporting quoteCalculatedShipping() (stub returning zero) and related types (OrderItemForQuote, AddressShape, ShippingQuote).

### Checkout procedures & utilities

- src/modules/checkout/server/procedures.ts, src/modules/checkout/server/utils.ts
  - Add product shipping parsing and ProductForShipping type; computeFlatShippingCentsForCart added; integrate shippingCents into session metadata/idempotency and session payload; switch money helpers to new money module.

### Cart store & persistence

- src/modules/checkout/store/use-cart-store.ts
  - Added ShippingSnapshot and shippingByProductId, public method setProductShippingSnapshot, migrations (persisted version bumped to v5) and composite-key collapse logic to preserve shipping snapshots.

### Checkout UI

- src/modules/checkout/ui/components/checkout-sidebar.tsx, src/modules/checkout/ui/views/checkout-view.tsx
  - Sidebar props changed to subtotalCents, shippingCents, totalCents, and onPurchaseAction; dev-time validation and toUsd helper added; view computes and passes cents values.

### Product UI & cart integration

- src/modules/products/ui/components/cart-button.tsx, src/modules/products/ui/components/shipping/shippingBadge.tsx, src/modules/products/ui/views/product-view.tsx, src/modules/products/ui/components/product-card.tsx
  - New ShippingBadge component; CartButton accepts shipping props and persists shipping snapshot when toggling cart; product view passes shipping props; tiny image class change.

### Server payload utilities

- src/lib/server/payload-utils/orders.ts
  - ShipmentGroup.carrier type changed to Carrier constant import; normalizeCarrier explicit exit branch added.
...
# Feature app wide auth consistency 04/06/26
...
# Standardize auth redirect 04/06/26

## Walkthrough

- Extracted the client-side moderation UI into a new ModerationInboxPage client component with three tabs and independent pagination/queries; converted the parent page.tsx to a server component that performs server-side auth checks and redirects unauthenticated users. Added a server-side auth redirect guard in the buyer dashboard.

## New Features

- Moderation inbox dashboard with three tabs ("Waiting Review", "Removed for Policy", "Open Appeals"), per-tab pagination, loading states, and detailed error/empty views.

## Improvements

- Moderation UI split: client-rendered inbox component with server-side page handling session gating and sign-in redirects.
- Buyer dashboard now enforces server-side auth and redirects to admin login when unauthorized.
...
- src/app/(app)/staff/moderation/moderation-inbox-client.tsx
  - New client component ModerationInboxPage: three tabs ("Waiting review", "Removed for policy", "Open Appeals"), two parallel tRPC queries (inbox gated, removed non-gated), per-tab pagination & page-size state, placeholderData for paging, delayed LoadingState, distinct error handling (401/403 vs others), client-side redirect on 401.

### Moderation page (server)

- src/app/(app)/staff/moderation/page.tsx
  - Converted page from client to async server component.
  - Calls caller.auth.session() server-side, redirects unauthenticated users to /sign-in?next=/staff/moderation on UNAUTHORIZED, rethrows other errors, and delegates UI to the client ModerationInboxPage.
